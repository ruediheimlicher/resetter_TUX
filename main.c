...
static volatile uint8_t resetdelay = 0;
...
uint16_t print_webpage_config(void)
{
        ...
        plen=fill_tcp_data_p(buf,plen,PSTR("<pre><a href=/>[home]</a></pre>\n"));
...
void print_webpage_relay(uint8_t on)
{
        ...
        pl=fill_tcp_data_p(buf,pl,PSTR("<pre><a href=/>[home]</a> <a href=/sw>[refresh]</a></pre>\n"));
...
pt=ms-((m+d*i)*60*1000);\
l.setTime(pt);\
dw(l.getFullYear()+\"-\"+pad(l.getMonth()+1)+\"-\"+pad(l.getDate())+\" \"+pad(l.getHours())+\":\"+pad(l.getMinutes())+\";\"+g1[i]+\";\"+g2[i]+\";\"+g3[i]+\";\");\
}\
document.close();\
}\n\
"));
...
uint16_t print_webpage_sensordetails(void)
{
        ...
        plen=http200ok();
        plen=print_line_mobile_webpage(plen);
        plen=fill_tcp_data_p(buf,plen,PSTR("<pre><a href=/sw>[switch]</a> <a href=/c>[cfg]</a> <a href=/g>[graph]</a> <a href=/>[refresh]</a></pre>\n"));
        plen=fill_tcp_data_p(buf,plen,PSTR("<h2>Current conditions</h2>"));
        ...
        // sht11:
        plen=fill_tcp_data_p(buf,plen,PSTR("<b>Indoor</b>\n"));
                //
        ...
        plen=fill_tcp_data_p(buf,plen,PSTR("<b>Outdoor</b>\n"));
...
// takes a string of the form command/Number and analyse it (e.g "?ri=24&pw=secret HTTP/1.1")
...
int main(void){
   ...
   /*
        ...
        gMeasurementTimer=0; // the above may take a while and we need to start with zero
    */
        while(1)
        {
           if (resetdelay > 8)
           {
              PORTD&= ~(1<<PORTD7); //Transistor sicher OFF
              resetdelay = 0;
           }
